...
export default function ShapeNode({ id, data, selected, width, height }) {
  ...
  return (
    <div ref={nodeRef} style={{
      ...
      <Handle type="target" id="t-t" position={Position.Top} style={{ background: 'transparent', width: 20, height: 20, top: -10, left: '50%', transform: 'translateX(-50%)', position: 'absolute' }} />
      ...
      {(data.connectMode || selected) && (
        <>
          ...
          <Handle type="source" id="l-s" position={Position.Left} style={{ background: '#3b82f6', width: 10, height: 10 }} />
          <Handle type="source" id="r-s" position={Position.Right} style={{ background: '#3b82f6', width: 10, height: 10 }} />
        </>
      )}
